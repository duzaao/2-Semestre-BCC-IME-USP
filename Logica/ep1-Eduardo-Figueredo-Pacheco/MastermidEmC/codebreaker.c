  ...
  for( i=0; i < _breaker.nslots; i++ ) {
    ...
    /* se for 2 */
      if( feedback[i]  == 2) {
        ...
        Lits1 = malloc( sizeof(int)*(_breaker.nslots-1));
        k=0;
        for( j=0; j < _breaker.nslots; j++ ){
          if( j!= i ){
            
            Lits1[k] = slot_color_to_var(j, color);
            k++;

          }
        }
        add_clause( _breaker.nslots-1, Lits1 );
    }
