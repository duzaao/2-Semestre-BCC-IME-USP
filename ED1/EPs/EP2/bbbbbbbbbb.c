...
int LugaresVazios_linha(matriz *tabuleiro, int m, int n ,int linha, int coluna, char **M, int numero, int* tamanho, item s, pilha*p){
...
*/
    ...
    for(s->ultima_palavra = 0; s->ultima_palavra < numero; s->ultima_palavra++){
        if(tamanho[s->ultima_palavra] == cont && match(palavra,M[s->ultima_palavra]) && !ok){
            

            /*printf("LINHA : %s  ", M[s->ultima_palavra] );*/
            ok =1;
            

            if(s->nao_usadas[s->ultima_palavra]){
            for(i = 0; i< tamanho[s->ultima_palavra];i++){
                
                if(s->nao_usadas[s->ultima_palavra]) empilha(p,s->ultima_palavra); /**/
...
int LugaresVazios_coluna(matriz *tabuleiro, int m, int n ,int linha, int coluna, char **M, int numero, int* tamanho, item s, pilha *p){
            ...
            if(s->nao_usadas[s->ultima_palavra]){
                ...
                for(i = 0; i< tamanho[s->ultima_palavra];i++){
                    ...
                    if(palavra[i] == '0'){
                        ...
                        escrever_tab(tabuleiro,y++,coluna,M[s->ultima_palavra][i]);
    ...
    for (i=0;i<linhas;i++){
        for(j=0;j<colunas;j++){
            ...
            if (retorna(&tabuleiro,i,j) != '*'){
                ...
                /*printf("(%d, %d) L: %d",i,j, LugaresVazios_linha(&tabuleiro, colunas, linhas, i,j,matriz_de_palavras,num_palavras,tamanho,&s));
                printf("  C: %d\n\n", LugaresVazios_coluna(&tabuleiro, linhas, colunas, i,j,matriz_de_palavras,num_palavras,tamanho,&s));
                */
            /*else recursão;*/
