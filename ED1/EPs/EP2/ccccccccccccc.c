...
int backtracking(matriz *tabuleiro, int m, int n ,int linha, int coluna, char **M, int numero, int* tamanho, item s, pilha *p, int *nao_usadas){
    int i;
    s = desempilha(p);


    nao_usadas[s->ultima_palavra] = 1;
    if(s->modo == 0){
            printf("AA");

        for(i = 0; i < strlen(M[s->ultima_palavra]); i++){
            escrever_tab(tabuleiro,s->coord_lin,s->coord_col+i,s->space[i]);
        }
    }
    else {
            printf("BB");
        ...
        for(i = 0; i < strlen(M[s->ultima_palavra]); i++){
            escrever_tab(tabuleiro,s->coord_lin +i ,s->coord_col,s->space[i]);
        }
    }

    return s->ultima_palavra;
    ...
    for (i=0;i<linhas;i++){
        for(j=0;j<colunas;j++){
            ...
            if (retorna(&tabuleiro,i,j) != '*'){
                ...
                ok = 0;
                ultima = 0;
                ok += LugaresVazios_linha(&tabuleiro, colunas, linhas, i,j,matriz_de_palavras,num_palavras,tamanho,s, p,nao_usadas,ultima);
    
    
     /*           while(!ok){
            

                ultima = backtracking(&tabuleiro, linhas, colunas, i,j,matriz_de_palavras,num_palavras,tamanho,s,p,nao_usadas);
                ok += LugaresVazios_linha(&tabuleiro, colunas, linhas, i,j,matriz_de_palavras,num_palavras,tamanho,s, p,nao_usadas,ultima);
                if(pilhaVazia(p)){
                           printf("tabuleiro : \n");
                print_tab(&tabuleiro);
                ...
                ok = 0;
                ultima = 0;
                ok += LugaresVazios_coluna(&tabuleiro, linhas, colunas, i,j,matriz_de_palavras,num_palavras,tamanho,s,p,nao_usadas,ultima);
        ...
        while(!pilhaVazia(p)){ 
            s = desempilha(p);
            printf("\n\n(%d,%d): [%s] - - > [%s]\n\n", s->coord_lin,s->coord_col,s->space ,matriz_de_palavras[s->ultima_palavra]);
