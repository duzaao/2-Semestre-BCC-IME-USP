...
/* FIM do FILA*/
    ...
    ok += LugaresVazios_coluna(tabuleiro, m, n, linha,coluna,M,numero,tamanho,s,p,nao_usadas,ultima);
...
printf("\n(%d,%d)%s ",linha,coluna,palavra);
    if(strlen(palavra)<2) ok =1;
    
   for(k = ultima; k < numero; k++){
    
        printf("-->%s/%s --> %d -->MATCH?: %d\n",M[k],palavra,ok,match(palavra,M[k]));
...
if(ok) return 1; 
if(!pilhaVazia(p)) ok = backtracking(tabuleiro,m,n,linha,coluna,M,numero,tamanho,p,nao_usadas);
return ok;
...
int backtracking(matriz *tabuleiro, int m, int n ,int linha, int coluna, char **M, int numero_palavras, int* tamanho, pilha *p, int *nao_usadas){
    ...
    printf("palavra:%s (%d,%d) modo: %d , %s", M[x->ultima_palavra],x->coord_col,x->coord_lin,x->modo,x->space);
    ...
    if(x->modo == 0){
        ...
        if(x->ultima_palavra+1 == numero_palavras) return 0;
        ...
        if(x->ultima_palavra+1 == numero_palavras) return 0;
        printf("\nAQUI: (%d,%d)%s -- %s\n",x->coord_lin,x->coord_col, M[x->ultima_palavra], M[x->ultima_palavra+1]);
    ...
    for (i=0;i<linhas;i++){
        for(j=0;j<colunas;j++){
            ...
            if (retorna(&tabuleiro,i,j) != '*'){
                ...
                ok = 0;
                ultima = 0;
                ok = LugaresVazios_linha(&tabuleiro, linhas, colunas, i,j,matriz_de_palavras,num_palavras,tamanho,s, p,nao_usadas,ultima);
