...
celula * buscaNivel(celula * x, item chave, int k) {
  ...
  if (chave == x->chave)
    return x;
  if (x->prox[k] == NULL || chave < x->prox[k]->chave) { // Se o número X é menor que o observado, vamos para tras (k-1);
            if (k > 0)
              return buscaNivel(x, chave, k - 1);   // desce o nível (?)
            else
              return NULL;
  }
  return buscaNivel(x->prox[k], chave, k);   // Agora, se é maior, vê-se o próximo no mesmo nível;
}



celula * busca(celula* x, item chave, int k){

  if (x==NULL) return NULL;
  if (chave == x->chave ) return x;
  if( x->prox[k] == NULL || chave < x->prox[k]->chave){
    if(k>0) return busca(x,chave,k-1);
    else return NULL;
  }
  return busca(x->prox[k],chave,k);
}
...
void insercaoCelula(celula * inicio, celula * x, int k) {
  ...
  if (inicio->prox[k] == NULL || chaveBuscada < inicio->prox[k]->chave) {// vamos descer de nível senaão (else )
    if (k < x->qtdNiveis) {
