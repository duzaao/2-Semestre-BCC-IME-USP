...
def equal(u,v):
    ...
    """
    assert u.D == v.D
    if (False in [u[k] == v[k] for k in u.f]): return False
    else: return True
    pass
...
def add(u,v):
    ...
    """
    assert u.D == v.D
    return Vec(u.D,{k: u[k] + v[k] for k in u.f if k in v.f})
    pass
...
def dot(u,v):
    ...
    """
    assert u.D == v.D
    return sum([u[k]*v[k] for k in u.f if k in v.f]) 
    pass
...
def neg(v):
    ...
    True
    """
    return Vec(v.D, {k:-1*v[k] for k in v.f})
    pass
